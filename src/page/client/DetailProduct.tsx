...
function ProductDetailClient() {
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selected =
      product?.volume.find((v) => v.type === e.target.value) || null;
  ...
  const handleBuyNow = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    nav("/payment", {
      state: { cartItems: [newItem], totalPrice: newItem.total },
    });
  };
  ...
  if (loading)
    return (
      <div className="text-center py-10">Đang tải chi tiết sản phẩm...</div>
    );
  if (error)
